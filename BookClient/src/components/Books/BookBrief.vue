  ...
  <div v-if="props.inBook" ref="bookBriefDiv" class="book-brief">
    ...
    <div class="book-information">
      <div class="book-information-top">
        <h1 class="">{{props.inBook.title}}</h1>
          ...
          <div class="author-information">
            ...
            <div class="author-other-information">
              ...
              <div class="author-dynasty">
                <p>朝代</p>
                <em class="color-font-card">{{ author.dynasty }}</em>
              </div>
            </div>
          </div>
        </div>
...
const getBookAuthors = (book: Book)=>{
  ...
  if(book.authors && book.authors.length){
    for(var author of book.authors){
      ...
      if(author.dynasty){
        htmlString += `<span>[${author.dynasty}]</span>`;
      }
      var type: string = author.type === undefined? '著' : author.type;
      htmlString += `<span class="writer">${author.name}</span>&nbsp;${type}`;
      firstItem = false;
    }
  }
  if(book.category){
    htmlString += `&nbsp;&nbsp;&nbsp;｜&nbsp;&nbsp;&nbsp;`;
    htmlString += `<span>${book.category}</span>`;
    if(book.subCategory){
      htmlString += `&nbsp;<span class="dot">·</span>&nbsp;`;
      htmlString += `<span>${book.subCategory}</span>`;
    }
  }
  return htmlString;
}
